...
func resourcePagerDutyAddonCreate(d *schema.ResourceData, meta interface{}) error {
	...
	req := buildAddonStruct(d)

	log.Printf("[INFO] Creating PagerDuty add-on %s", req.Name)

	addon, err := client.InstallAddon(req)
	if err != nil {
		return err
	}
...
func resourcePagerDutyAddonUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if _, err := client.UpdateAddon(d.Id(), req); err != nil {
		return err
	}
